CREATE TABLE IF NOT EXISTS `CUSTOMER` (
    `user_id`    VARCHAR(70) PRIMARY KEY COMMENT '회원아이디',
...
CREATE TABLE IF NOT EXISTS `PRODUCT` (
    ...
    `price`      VARCHAR(255) NOT NULL COMMENT '상품구분',
...
CREATE TABLE IF NOT EXISTS `CART` (
    ...
    `user_id`      VARCHAR(70) NOT NULL COMMENT '회원아이디',
    `is_activated` VARCHAR(255) DEFAULT TRUE NOT NULL COMMENT '활성여부',
...
CREATE TABLE IF NOT EXISTS `ORDER_PRODUCT` (
    ...
    `product_id`       INT NOT NULL COMMENT '상품번호',
    `count`            VARCHAR(255) NOT NULL COMMENT '주문수량',
